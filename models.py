...
class EmoNeXt(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)              # LayerNorm trên channel dim
        x = x.permute(0, 3, 1, 2)
        return x # (B, C, H, W) giữ lại spatial dimensions để attention hoạt động trên đó
    # Change
    ...
    def forward(self, x, labels=None):
        ...
        attended_x, weights = self.attention(x)  # weights: (B, N, N)
        x = attended_x.mean(dim=1)            # (B, C) global pooling sau attention
        logits = self.head(x)
        if labels is not None:
            mean_attention_weight = weights.mean(dim=-1, keepdim=True)  # (B, 49, 1)
            attention_loss = ((weights - mean_attention_weight) ** 2).mean()
            # Change
            # loss = F.cross_entropy(logits, labels, label_smoothing=0.2) + attention_loss
            loss = F.cross_entropy(logits, labels, label_smoothing=0.1) + 0.1 * attention_loss
