...
class Trainer:
    def __init__(
        ...
        early_stopping_patience: int = 20, # change: 12 -> 20 epochs 
        execution_name=None,
        lr: float = 1e-5, # choose parameter from cmd line: 3e-5
        amp: bool = False,
        ema_decay: float = 0.99,
        ema_update_every: int = 16,
        gradient_accumulation_steps: int = 1, # choose parameter from cmd line: 2 for opt gpu memory
        ...
        #     self.device
        # )
        # change: add warmup and cosine scheduler, update ema every step
        ...
        warmup_scheduler = LinearLR(self.optimizer, start_factor=0.1, total_iters=warmup)
        cosine_scheduler = CosineAnnealingLR(self.optimizer, T_max=total_steps - warmup, eta_min=1e-7)
    ...
    def train_epoch(self):
        ...
        for batch_idx, data in enumerate(self.training_dataloader):
            ...
            inputs = inputs.to(self.device) # batch_size, 3, 224, 224
            labels = labels.to(self.device)

            # Change
            with torch.autocast(self.device.type, enabled=self.amp):
                # use model's forward to get predictions, logits, and loss
                predictions, _, loss = self.model(inputs, labels) # output: class indices, logits, scalar
    ...
    def val_epoch(self):
        self.model.eval() # opt in feature: need to set ema instead of model to eval for validation and test, because we want to use the averaged weights for better performance
        ...
        for batch_idx, (inputs, labels) in enumerate(self.validation_dataloader):
            ...
            labels = labels.to(self.device)
            # change: use model's forward to get predictions, logits, and loss
            with torch.autocast(self.device.type, enabled=self.amp):
...
if __name__ == "__main__":
    ...
    train_transform = transforms.Compose(
        [
            ...
            # transforms.RandomVerticalFlip(), # Change - not suitable for FER
    ...
    val_transform = transforms.Compose(
        [
            ...
            transforms.Resize(236),
            # transforms.RandomCrop(224),
            # change to center crop for validation and test
    ...
    val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False) # Change: batch_size=32 (optional to device memory)
